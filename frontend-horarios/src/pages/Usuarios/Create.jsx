...
const Create = () => {
  ...
  const loadFormData = async () => {
    try {
      setLoadingForm(true);
      const data = await usuarioService.getFormData();
      setRoles(data.roles || []);
    } catch (error) {
      console.error('Error al cargar datos del formulario:', error);
      setErrors({ form: 'Error al cargar los roles disponibles.' });
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.response?.data?.errors) {
        ...
        setErrors({ submit: error.response?.data?.message || 'Error al crear el usuario' });
      }
    } finally {
      setLoading(false);
    }
