...
const Create = () => {
  ...
  const loadFormData = async () => {
    try {
      setLoadingForm(true);
      const data = await grupoService.getFormData();
      setMaterias(data.materias || []);
      setDocentes(data.docentes || []);
      setGestiones(data.gestiones || []);
    } catch (error) {
      console.error('Error al cargar datos del formulario:', error);
      setErrors({ form: 'Error al cargar los datos necesarios para el formulario.' });
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await grupoService.createGrupo(formData);
      navigate('/grupos');
    } catch (error) {
      if (error.response?.data?.errors) {
        ...
        setErrors({ form: 'Error al crear el grupo. Por favor, intenta nuevamente.' });
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-5">
            ...
            <p className="text-blue-100 text-sm mt-1">Complete los datos del nuevo grupo académico</p>
          </div>
